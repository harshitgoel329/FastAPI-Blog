...
async def test_get_posts_empty(client: AsyncClient):
    ...
    response = await client.get("/api/posts") # here we are caling the cliet from the conftest.py file 
...
async def test_get_posts_not_found(client: AsyncClient):
    ...
    response = await client.get("/api/posts/999") # here we are caling the cliet from the conftest.py file 
